...
function asynchronize(pr, opts, logger, parsePart, printNode) {
    ...
    function bindAsync(left,arg) {
        ...
        if (opts.noRuntime) {
            if (arg) {
                ...
                left.body.body = parsePart("try {$:0} catch($2) {return $1($2)}",[cloneNode(left.body),arg,ident('$boundEx')]).body ;
        ...
        if (opts.$runtime) {
	        	if (arg) {
	        		return parsePart("$0.$1.call($0,this,$2)",[genIdent.runtime,left,genIdent.asyncbind,arg]).expr;
	        	} else {
	        		return parsePart("$0.$1.call($0,this)",[genIdent.runtime,left,genIdent.asyncbind]).expr;
	        	}
        } else {
	        	if (arg) {
	        		return parsePart("$0.$1(this,$2)",[left,genIdent.asyncbind,arg]).expr;
	        	} else {
	        		return parsePart("$0.$1(this)",[left,genIdent.asyncbind]).expr;
	        	}
        }
    }
    ...
    */
    ...
    function thisCall(name, args) {
        if (typeof name === 'string')
            name = ident(name);
        var n = parsePart("$0.call($1)",[name,[{"type": "ThisExpression"}].concat(args || [])]).expr ;
        name.$thisCall = n;
    ...
    function mapCondOp(ast, state) {
        ...
        treeWalk(ast, function (node, descend, path) {
            ...
            if (node.type === 'ConditionalExpression' && (containsAwait(node.alternate) || containsAwait(node.consequent))) {
                ...
                var xform = internalIIAFE(parsePart("if ($0) return $1 ; return $2",[node.test,node.consequent,node.alternate]).body);
                coerce(node, xform);
            }
        }, state);
        return ast;
  ...
  if (x) { y; return $more(); } function $more() { more... } $more() ;
        ...
        if (node.$needsMapping) {
            ...
            if ('index' in ref) {
                ...
                if (afterTry.length) {
                    ...
                    continuation = parsePart("return $0()",
            ...
            if (node.finalizer) {
                ...
                var callFinally = parsePart("return $0()",
                    [node.finalizer ? deferredFinally(node, ident(ctnName)) : ident(ctnName)]).body[0] ;
    ...
    function transformForIn(node, path) {
        ...
        var declVars = parsePart("var $0,$1 = [];for ($0 in $2) $1.push($0)",[idx,inArray,node.right]).body ;
        var loop = parsePart("for ($0; $1.length;){ $2 = $1.shift(); $:3 ; }",[
            node.left,
            inArray,
    ...
    function iterizeForOf(node, path) {
        ...
        node.test = parsePart("!($0[1] = $0[0].next()).done && (($1 = $0[1].value) || true)",[iterator,index]).expr ;
        delete node.left;
        delete node.right;
    }
    ...
    function mapLoops(node, path, down) {
        ...
        if (!node.$mapped && examine(node).isLoop && contains(node,mappableLoop)) {
          ...
          if (opts.noRuntime) {
            ...
            invokeIterate = parsePart("($0.$1.trampoline(this,$2,$3,$4,$6)($5))",[
            		opts.$runtime ? genIdent.runtime : ident('Function'),
                genIdent.asyncbind,
    ...
    function spawnBody(body, deferExit) {
        ...
        return parsePart("{ return (function*($return,$error){ $:body }).$asyncspawn($promise,this) }",{
    ...
    function fixSuperReferences(ast,engine) {
        return treeWalk(ast, function (node, descend, path) {
            ...
            if (node.type === 'ClassDeclaration' || node.type === 'ClassExpression') {
                ...
                if (node.$superID) {
                    var method = parsePart("(function($field) { return super[$field] })",{ field:ident("$field") }).expr ;
                    if (opts.babelTree) {
...
var notNodeKeys = { start:true, end: true, loc: true, range: true, type: true } ;
    ...
    if (n && n.type) {
		var keys = Object.keys(n).filter(function(k) { 
			return k[0]!='$' && k[0]!='_' 
				...
				&& typeof n[k]==='object' 
				&& n[k] 
			    && (typeof n[k].type==='string' || Array.isArray(n[k]))
		}) ;

		keys.forEach(function(member){
			if (Array.isArray(n[member])) {
				n[member].forEach(function(node,index){down(node,member,index)}) ;
			} 
			else
...
module.exports = {
    ...
    transform: function(pr, opts, helpers){
    		return asynchronize(pr, opts, 
    				helpers.logger, 
    				partialParser(helpers.parse),
    				helpers.printNode) ;
    }
};
